...
def run_pvt_corner(
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="autoanalog-pvt-") as directory:
...
def run_pvt(
    ...
    try:
        ...
        low_level = float(np.percentile(measurement_v, 5))
        high_level = float(np.percentile(measurement_v, 95))
        ...
        phase = np.mod(time_s - time_s.min(), ui)

        # For each phase position, determine whether the waveform
        # has a valid eye opening.
        phase_bins = np.linspace(0.0, ui, 201)
        eye_open = np.zeros(len(phase_bins) - 1, dtype=bool)
  ...
  def _inject_parameters(
    ...
    rendered = self.template.replace("{VDD}", self._spice_value(vdd))
    ...
    if self.pdk_model_path:
        ...
        if self.pdk_corner_path:
            corner = pvt_process if pvt_process is not None else self.pdk_corner
    ...
    rendered = rendered.replace(
        "{I_bias/2}",
        self._spice_value(parameters["I_bias"] / 2.0),
    )
